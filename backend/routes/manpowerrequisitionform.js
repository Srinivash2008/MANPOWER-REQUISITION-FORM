...
router.get('/getmanpowerrequisitionbyid/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT mr.id AS mr_id,ed.id AS depart_id, depart, employment_status, designation, num_resources, requirement_type, project_name, projection_plan, ramp_up_file, replacement_detail, ramp_up_reason, job_description, education, experience, ctc_range, specific_info, hiring_tat_fastag, hiring_tat_normal_cat1, hiring_tat_normal_cat2, mrf_number, tat_agreed, delivery_phase, hr_review, requestor_sign, director_sign, status, hr_status, director_status, hr_comments,  director_comments, created_by, created_at, mrq.query_name_hr, emp.emp_name AS emp_name,mrq.query_name_director,mrq.Director_Query_Answer,mrq.HR_Query_Answer, mrq.query_pid, mrq.query_created_by FROM manpower_requisition AS mr JOIN employee_depart AS ed ON ed.id = mr.department LEFT JOIN manpower_requisition_query as mrq ON mr.id = mrq.query_manpower_requisition_pid LEFT JOIN employee_personal as emp ON emp.employee_id = mr.created_by WHERE mr.id = ? AND mr.isdelete = "Active" ORDER BY mrq.query_pid DESC LIMIT 1`, [id]);
...
router.get('/manager-mrf-counts/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const overallSelect = isHr ? `
            ...
            COALESCE(SUM(CASE WHEN hr_status = 'HR Approve' AND director_status = 'Approve' THEN 1 ELSE 0 END), 0) AS hrapproved_count,
        ...
        const response = {
            overall: {
                ...
                FH_total_count : counts.FH_total_count,
                dir_pending_count : counts.dir_pending_count,
                hrpending_count: counts.hrpending_count,
                hrapproved_count : counts.hrapproved_count,
                FH_completed_count : counts.FH_completed_count,
            },
            director_status: {
