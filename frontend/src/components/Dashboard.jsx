...
const Dashboard = () => {
  ...
  const counts = {
    ...
    approved: displayedMrfList.filter(m => (m.status === 'Approve' || m.status === 'HR Approve') && m.mrf_track_status !== 'Joined' && m.mrf_track_status !== 'Offered' && m.mrf_track_status !== 'IJP').length,
    ...
    approved_completed: displayedMrfList.filter(m => m.mrf_track_status === 'Joined' || m.mrf_track_status === 'Offered' || m.mrf_track_status === 'IJP').length,
  ...
  const overallStatusMetrics = [
    { status: 'Submitted', count: baseCounts.total, icon: (props) => <PendingIcon {...props} />, color: 'secondary' },
